...
mixin NotificationsHelper {
  ...
  void showError(String error, {BuildContext? context}) {
    if (kDebugMode) printIfDebugMode(error);
    ...
    context == null
        ? ElegantNotification.error(
            ...
            description:
                Text(error.trim(), style: const TextStyle(color: Colors.black)),
            toastDuration: const Duration(seconds: 5),
          )
        : ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
  ...
  void showSuccess(String success) => ElegantNotification.success(
        ...
        description:
            Text(success.trim(), style: const TextStyle(color: Colors.black)),
        toastDuration: const Duration(seconds: 5),
      );
